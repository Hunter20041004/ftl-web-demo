    ...
    <EntityPage<EventData>
      ...
      summary={(row) => (
        <div className="min-w-0">
          <p className="font-medium md:truncate"><span className="mr-2 font-mono text-xs text-muted-foreground">W{String(row.week).padStart(2, "0")} {String(row.date).slice(5)}</span><span className="mr-2 rounded-full bg-secondary px-2 py-0.5 text-xs text-primary">{kindLabel(String(row.kind ?? row.data.kind))}</span>{row.data.zh}</p>
