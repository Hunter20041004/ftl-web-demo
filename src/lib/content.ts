...
export type WeeklyStory = {
  ...
  facts: string[]; factsEn: string[];                    // 重點：關鍵數字與事實
  context: string; contextEn: string;                    // 背景：這件事之前發生什麼
  quote?: string; quoteEn?: string; quoteBy?: string; quoteByEn?: string; // 說法：一手來源裡的一句話
  why: string; whyEn: string;                            // 為什麼重要
  taiwan: string; taiwanEn: string;                      // 台灣視角
  watch: string[]; watchEn: string[];                    // 接下來看什麼：具體事件，不是預測
  term?: string; termEn?: string;                        // 名詞：只在需要時
...
export const weekly: WeeklyIssue[] = [
  {
    ...
    lede: "這週兩家美國公司都在買「把錢送進銀行帳戶」的資格，一家用收購、一家用申請執照。台灣這邊，金管會公布 7 月電子支付帳戶數，又多了 38 萬人。",
    ledeEn: "Two US companies spent this week buying the right to move money into bank accounts, one by acquisition and one by applying for a charter. In Taiwan, the FSC’s July figures show another 380,000 e-payment accounts.",
    stories: [
      {
        title: "Circle 以 4 億美元收購新加坡跨境支付公司 Tazapay",
        titleEn: "Circle to acquire Singapore cross-border payments firm Tazapay for $400M",
        lede: "發行 USDC 的 Circle 在 9 月 8 日宣布，用 4 億美元的股票買下新加坡的 Tazapay。Tazapay 替企業做跨境收付款。交易要等新加坡金管局點頭，預計 2027 年完成。",
        ledeEn: "Circle, the issuer of USDC, said on September 8 it will buy Singapore’s Tazapay for $400 million in stock. Tazapay handles cross-border payments for businesses. The deal needs approval from the Monetary Authority of Singapore and is expected to close in 2027.",
        facts: ["Tazapay 年化交易量超過 250 億美元，付款通路涵蓋 100 多個市場，銀行與金融科技夥伴 60 多家", "這些交易量裡，大約六成已經用穩定幣結算", "兩家不是第一次合作：2025 年就有合作關係，Tazapay 的 B 輪募資由 Circle 領投", "這是 Circle 歷來最大的交易之一。Circle 2025 年 6 月在紐約證交所上市，今年 7 月拿到美國 OCC 的國家信託銀行執照"],
        factsEn: ["Tazapay: over $25 billion in annualised volume, payout rails in 100+ markets, 60+ banking and fintech partners", "About 60% of that volume already settles in stablecoins", "Not a first date: the two partnered in 2025 and Circle led Tazapay’s Series B", "One of Circle’s largest deals. Circle listed on the NYSE in June 2025 and received an OCC national trust bank charter in July"],
        context: "Circle 的主力產品是 USDC，一種價格釘住美元的加密貨幣。它過去幾年的問題一直是同一個：發幣容易，但要讓幣真的變成某個國家某家銀行帳戶裡的錢，需要在每個市場有銀行關係、有牌照、有付款通路。Tazapay 手上有的就是這些。",
        contextEn: "Circle’s main product is USDC, a cryptocurrency pegged to the US dollar. Its problem for years has been the same: issuing a coin is easy, but turning that coin into money in a bank account in a given country needs banking relationships, licences and payout rails in each market. That is what Tazapay has.",
        why: "市場上不缺穩定幣，缺的是最後一哩。買 Tazapay 等於買下 100 多個市場的付款出口，以後 USDC 到了東南亞或新興市場，不用再靠別人的管線落地。TD Cowen 的分析師認為整合風險相對有限，因為兩家本來就在合作。",
        whyEn: "The market has no shortage of stablecoins; what it lacks is the last mile. Buying Tazapay buys payout exits in more than 100 markets, so USDC no longer needs someone else’s pipes to land in Southeast Asia or emerging markets. TD Cowen analysts see relatively limited integration risk because the two were already working together.",
        taiwan: "台灣的《虛擬資產服務法》6 月 30 日三讀，穩定幣子法最快 2027 年第一季上路。法規追上之後，台灣企業付海外貨款要是能選穩定幣，背後跑的很可能就是 Circle 這種公司加上一家像 Tazapay 的通路商。",
        taiwanEn: "Taiwan’s Virtual Asset Services Act passed on June 30 and the stablecoin rules could take effect in Q1 2027. Once the rules catch up, a Taiwanese company paying an overseas supplier in stablecoins would most likely be running on a Circle-type issuer plus a rails provider like Tazapay.",
        watch: ["新加坡金管局的核准，以及交易是否照 2027 年的時程完成", "Circle 的支付網路 CPN 接上 Tazapay 通路後，先開哪些市場"],
        watchEn: ["MAS approval and whether closing stays on the 2027 schedule", "Which markets Circle’s payment network (CPN) opens first once Tazapay’s rails are plugged in"],
        quote: "穩定幣結算正在成為全球經濟的核心基礎設施。把 USDC 和 Tazapay 的銀行關係、在地付款通路與機構客戶結合起來，會加快 USDC 在全球的採用。",
        quoteEn: "Stablecoin settlement is becoming core infrastructure in the global economy, and combining USDC with Tazapay’s world-class banking relationships, local payout rails, and institutional customer base will accelerate worldwide USDC adoption.",
        quoteBy: "Jeremy Allaire，Circle 共同創辦人兼執行長",
        quoteByEn: "Jeremy Allaire, Circle co-founder and CEO",
        term: "穩定幣：價格釘住美元等法定貨幣的加密貨幣。拿來付款比拿來投資多。",
        termEn: "Stablecoin: a cryptocurrency pegged to a fiat currency such as the US dollar. Used for payments more than for investing.",
        sources: [
      ...
      },
      {
        ...
        lede: "Square 收款機和 Cash App 的母公司 Block，9 月 8 日宣布向美國聯邦銀行監理機關 OCC 申請一張國家信託銀行執照，名字叫 Builders Bank & Trust。",
        ledeEn: "Block, the parent of Square terminals and Cash App, announced on September 8 that it has applied to the OCC, the US federal bank regulator, for a national trust bank charter under the name Builders Bank & Trust.",
        facts: ["這家銀行不收存款、不放款、沒有存款保險。只做保管與信託，包括替客戶保管比特幣和穩定幣", "預定的總裁兼執行長是 Lee Woolley，現在負責 Block 的數位資產策略，之前是美國財政部聯邦信用合作社的執行長", "Block 已經有一家銀行：2021 年在鹽湖城成立的 Square Financial Services，用的是猶他州 2020 年核發的工業銀行執照", "核准時間沒有公布"],
        factsEn: ["The bank would take no deposits, make no loans and carry no deposit insurance. Custody and fiduciary services only, including custody of bitcoin and stablecoins", "President and CEO designate is Lee Woolley, who leads digital asset strategy at Block and formerly ran the Treasury Department Federal Credit Union", "Block already owns a bank: Square Financial Services, set up in Salt Lake City in 2021 under a Utah industrial loan charter granted in 2020", "No approval timeline given"],
        context: "今年 5 月，美國總統簽了一道行政命令，要聯邦機關簡化金融科技公司申請銀行執照的流程。之後排隊的人很多：Stripe 旗下的 Bridge 2 月拿到有條件核准，Circle 7 月拿到正式核准，Revolut 上週拿到有條件核准；Affirm 和 PayPal 去申請州的工業銀行執照，Klarna 兩個月前拿到猶他州執照。也有被拒的，英國的 Wise 7 月被 OCC 打回票。",
        contextEn: "In May the US president signed an executive order telling federal agencies to streamline bank-charter applications from fintechs. The queue has been long since: Stripe’s Bridge got conditional approval in February, Circle final approval in July, Revolut conditional approval last week; Affirm and PayPal went for state industrial bank charters, and Klarna got a Utah charter two months ago. Not everyone gets in: the OCC turned down the UK’s Wise in July.",
        why: "Block 多申請一張執照，重點在「聯邦」兩個字。保管數位資產這件事目前是各州各管，拿到 OCC 的信託執照後就是全國一套規則。先當一家只保管、不放款的銀行，再決定要不要走更遠，這條路現在幾乎每家大型金融科技公司都在走。",
        whyEn: "The point of Block’s extra charter is the word federal. Custody of digital assets is currently supervised state by state; an OCC trust charter means one national rulebook. Become a custody-only, no-lending bank first and decide later how far to go: nearly every large fintech is now on this path.",
        taiwan: "金管會今年在推銀行的虛擬資產保管業務試辦。美國是另外發一種執照，台灣是讓現有銀行先試。做法不同，要回答的問題一樣，就是誰可以合法替你保管幣，出事了誰負責。",
        taiwanEn: "The FSC is piloting virtual-asset custody at existing banks this year. The US issues a separate charter; Taiwan lets current banks try first. Different methods, same question: who may legally hold your coins, and who is liable when something goes wrong.",
        watch: ["OCC 對 Builders Bank & Trust 的決定，以及是否附條件", "Revolut 的有條件核准要滿足哪些條件，會是後面申請者的參考"],
        watchEn: ["The OCC’s decision on Builders Bank & Trust, and whether it comes with conditions", "What conditions Revolut must meet under its conditional approval, a template for later applicants"],
        sources: [
      ...
      },
      {
        ...
        lede: "金管會 9 月 10 日公布 7 月的電子支付統計。帳戶使用者 4,151 萬人，比 6 月多 38 萬人。交易、儲值、匯兌、餘額四個數字也都比上個月高。",
        ledeEn: "The FSC released July e-payment statistics on September 10. Users reached 41.51 million, up 380,000 from June. Transactions, stored value, remittances and balances all rose too.",
        facts: ["代理收付實質交易 330.1 億元，比 6 月多 33.6 億元", "收受儲值款項 335 億元（6 月 320 億元）；國內外小額匯兌 180.1 億元（6 月 172.7 億元）", "支付款項餘額 200.9 億元，多 3.7 億元", "經營電子支付的業者：專營 10 家、兼營 19 家", "同一份資料裡的信用卡：流通卡數 5,949 萬張，比上月少 32 萬張"],
        factsEn: ["Payment collection NT$33.01 billion, up NT$3.36 billion from June", "Stored value received NT$33.5 billion (NT$32.0 billion in June); small domestic and overseas remittances NT$18.01 billion (NT$17.27 billion)", "Outstanding balances NT$20.09 billion, up NT$370 million", "Operators: 10 dedicated e-payment institutions and 19 that run it alongside other business", "Same release, credit cards: 59.49 million cards in circulation, down 320,000 from the previous month"],
        context: "這份資料金管會每個月都出，叫「信用卡、現金卡及電子支付機構業務資訊摘要」，格式固定。電子支付帳戶指的是 LINE Pay Money、街口、悠遊付這一類可以儲值、轉帳、收付款的帳戶，跟悠遊卡那種儲值卡分開統計。",
        contextEn: "The FSC publishes this release every month under the title “Credit Card, Cash Card and Electronic Payment Institutions Business Summary”, always in the same format. E-payment accounts are the LINE Pay Money, JKOPay and EasyWallet kind that can store value, transfer and pay; stored-value cards such as EasyCard are counted separately.",
        why: "4,151 萬個帳戶比台灣人口還多，所以大部分人手上不只一個。業者的仗已經不是叫人開戶，是搶「每天用哪一個」。一個月多 33.6 億的代理收付，看的就是這件事。",
        whyEn: "41.51 million accounts is more than Taiwan’s population, so most people hold more than one. The fight is no longer about getting people to sign up but about which app gets used every day. The NT$3.36 billion monthly jump in payment collection is what that fight looks like.",
        taiwan: "要做台灣行動支付的報告或專案，這是最直接的一手資料。每月更新、每項數字都有上月對照，引用時寫「金管會 115 年 7 月電子支付機構業務資訊摘要」就可以。",
        taiwanEn: "For any report or project on mobile payments in Taiwan this is the most direct primary source: monthly, every figure with a prior-month comparison, citable as the FSC’s July 2026 e-payment summary.",
        watch: ["10 月上旬公布的 8 月數字，暑假結束後交易金額會不會回落", "儲值卡那邊 2 億 441 萬張的流通卡數，跟電支帳戶的成長速度誰快"],
        watchEn: ["The August figures due in early October, and whether transaction value drops after the summer break", "Stored-value cards at 204.41 million versus e-payment accounts: which grows faster"],
        sources: [
  ...
  },
  {
    ...
    lede: "金管會給了穩定幣子法一個時間。同一週，Stripe 找了人來做穩定幣卡。另外一條線是 Ramp：公司花在 AI 上的錢，開始有人幫忙管。",
    ledeEn: "The FSC put a date on Taiwan’s stablecoin rules. The same week, Stripe hired someone to build stablecoin cards. On a separate track, Ramp started managing what companies spend on AI.",
    stories: [
      {
        ...
        lede: "金管會主委彭金隆 9 月 2 日在台北的亞洲金融科技聯盟高峰會說，《虛擬資產服務法》的九項子法規正在訂，穩定幣草案最快明年第一季公告實施。",
        ledeEn: "FSC chair Peng Jin-lung said at the Asia FinTech Alliance summit in Taipei on September 2 that nine sets of rules under the Virtual Asset Services Act are being drafted, and the stablecoin draft could be announced and take effect as early as Q1 next year.",
        facts: ["《虛擬資產服務法》6 月 30 日三讀通過，共 56 條", "法律規定：台灣境內只有銀行可以發穩定幣，只能釘住法定貨幣，準備資產要足額並交付信託，發行要央行同意、金管會核准", "目前台灣市場上的穩定幣都是國外發的，主要是 USDT 和 USDC", "彭金隆說，國際上的討論已經從「該不該發展」變成「怎麼健全地發展」"],
        factsEn: ["The Virtual Asset Services Act passed on June 30 with 56 articles", "Under the Act only banks may issue stablecoins in Taiwan, pegged only to fiat, with full reserves held in trust, and issuance needs central bank consent and FSC approval", "Every stablecoin traded in Taiwan today is foreign-issued, mainly USDT and USDC", "Peng said the international debate has moved from whether to develop virtual assets to how to develop them soundly"],
        context: "這部法把過去只靠洗錢防制法登記的虛擬資產業者，改成事前許可制，要有最低資本、營業保證金和內控。穩定幣的部分是第一次寫進法律。但法條只給骨架，誰能申請、準備金怎麼查、多久申報一次，都在子法規裡，所以業者現在都在等這九項。",
        contextEn: "The Act moves virtual-asset businesses from simple anti-money-laundering registration to prior licensing, with minimum capital, a guarantee fund and internal controls. Stablecoins are written into law for the first time. But the Act is only a skeleton: who may apply, how reserves are checked and how often issuers report are all in the implementing rules, which is why the industry is waiting on these nine.",
        why: "這是第一次有官方時間表。想在台灣發穩定幣的銀行、想接穩定幣支付的業者，現在知道該在什麼時候準備好。同一場會上，台灣金融科技協會的王可言也提醒，穩定幣穩不穩，看的是準備金、贖回、技術和監理四件事有沒有人信。",
        whyEn: "It is the first official timetable. Banks that want to issue a stablecoin in Taiwan, and businesses that want to accept one, now know when to be ready. At the same event, Jennifer Wang of the Taiwan FinTech Association noted that a stablecoin is only as stable as the trust in its reserves, redemption, technology and regulation.",
        taiwan: "對想找相關實習或做專案的人，2027 年第一季前後是業者最缺人的時候：申請文件、內控、準備金報告都要有人做。",
        taiwanEn: "For anyone looking for an internship or a project in this area, the months around Q1 2027 are when firms will be shortest of hands: applications, internal controls and reserve reporting all need people.",
        watch: ["九項子法規的草案什麼時候預告，預告期通常 60 天", "哪一家銀行第一個遞件申請發行"],
        watchEn: ["When the nine draft rules go out for public comment (usually a 60-day period)", "Which bank files the first application to issue"],
        quote: "等子法規上路，台灣的虛擬資產市場會呈現全新的面貌。",
        quoteEn: "Taiwan’s virtual asset market will take on a completely new look once the rules are in place.",
        quoteBy: "彭金隆，金管會主委",
        quoteByEn: "Peng Jin-lung, FSC chair",
        sources: [
          ...
          { label: "中央社：立院三讀虛擬資產法，明定穩定幣發行依據（2026-06-30）", labelEn: "CNA: Legislature passes the Virtual Asset Services Act, sets the basis for stablecoin issuance (2026-06-30)", href: "https://www.cna.com.tw/news/aipl/202606300063.aspx", primary: false },
        ],
      },
      {
        title: "Stripe 延攬 Drew Turchin 負責穩定幣連結的支付卡業務",
        titleEn: "Stripe hires Drew Turchin to lead stablecoin-linked cards",
        lede: "Payments Dive 9 月 3 日報導，Stripe 找來 Drew Turchin 負責穩定幣卡。他之前在 Native Markets 和 Uniswap Labs 做業務開發。Stripe 沒有對此發表評論。",
        ledeEn: "Payments Dive reported on September 3 that Stripe has brought in Drew Turchin to lead stablecoin-linked cards. He previously ran business development at Native Markets and Uniswap Labs. Stripe declined to comment.",
        facts: ["穩定幣卡的運作：帳戶裡放穩定幣，刷卡時自動換成當地貨幣，任何收卡的地方都能用", "Stripe 網站引用 Artemis、Castle Island、Dragonfly 的研究：穩定幣卡月交易量 2024 年 15 億美元，2023 年 2.5 億美元", "Stripe 2024 年 10 月花 11 億美元買下穩定幣平台 Bridge；Bridge 後來又買了錢包公司 Privy", "今年 6 月，Stripe 和 Visa、Mastercard、美國運通、Adyen、Coinbase 等約 140 家公司一起宣布 Open USD 穩定幣"],
        factsEn: ["How a stablecoin card works: stablecoins sit in the account and convert to local currency at the point of sale, anywhere cards are accepted", "Research cited on Stripe’s site (Artemis, Castle Island, Dragonfly): stablecoin-card volume of $1.5 billion a month in 2024, from $250 million in 2023", "Stripe bought stablecoin platform Bridge for $1.1 billion in October 2024; Bridge later bought wallet company Privy", "In June Stripe joined Visa, Mastercard, American Express, Adyen, Coinbase and about 140 others to announce the Open USD stablecoin"],
        context: "Stripe 是線上收款的基礎設施公司，很多網站背後的刷卡都是它處理。它在穩定幣上已經連續下了三步棋：買 Bridge 拿發行和轉換的能力，加入 Open USD 拿一個大家共用的幣，現在找人做卡，把幣送到消費者手上。",
        contextEn: "Stripe is the payments infrastructure behind a large share of online checkouts. It has made three moves in a row on stablecoins: buying Bridge for issuance and conversion, joining Open USD for a shared coin, and now hiring for cards to put the coin in consumers’ hands.",
        why: "消費者不需要知道自己在用穩定幣。卡片長得跟原來一樣，收單店家也不用改任何東西，換掉的是背後的結算。這大概是穩定幣最有機會走進日常的一條路，因為它不要求任何人改變習慣。",
        whyEn: "Consumers never need to know they are using a stablecoin. The card looks the same and merchants change nothing; what changes is the settlement underneath. It may be the most likely route for stablecoins into daily life, because it asks nobody to change a habit.",
        taiwan: "這種卡在台灣還發不了，要等穩定幣子法規（見上一則）定案，而且法律規定境內只有銀行能發幣。所以如果台灣出現穩定幣卡，發卡的會是銀行，不是 Stripe 這類公司。",
        taiwanEn: "Such cards cannot be issued in Taiwan yet: the stablecoin rules (previous story) must be finalised, and only banks may issue coins domestically. So if a stablecoin card appears in Taiwan, a bank will be behind it, not a Stripe-type company.",
        watch: ["Stripe 何時正式公布穩定幣卡產品，以及和哪家卡組織合作", "Open USD 預定 2026 年上線，時程有沒有變"],
        watchEn: ["When Stripe formally announces the card product and which network it runs on", "Open USD is slated for 2026; whether that date holds"],
        quote: "卡片正在解鎖下一步，讓這些餘額能在日常生活中立刻花掉，任何收卡的地方都行。",
        quoteEn: "Cards are helping unlock the next step, making those balances instantly spendable in everyday life, anywhere cards are accepted.",
        quoteBy: "Drew Turchin，在 LinkedIn 上的貼文",
        quoteByEn: "Drew Turchin, in a LinkedIn post",
        sources: [
      ...
      },
      {
        ...
        lede: "紐約的企業卡與費用管理公司 Ramp，8 月 19 日推出 Router。公司可以看到自己在各家 AI 模型上花了多少錢，把工作分派給便宜或快的模型，還能設上限。",
        ledeEn: "Ramp, the New York corporate-card and expense-management company, launched Router on August 19. Companies can see what they spend on each AI model, route work to cheaper or faster ones, and set caps.",
        facts: ["支援 OpenAI、Anthropic、Google、Meta 等模型，依成本、速度或表現決定送去哪一家，產品裡會顯示分派的理由", "Ramp 內部用了三年才對外推出；自家 AI 支出從 2025 年夏天到現在成長 21 倍", "定價還沒定。應用 AI 負責人 Veeral Patel 說，目前只想先把路由做穩", "同一個月，Stripe 以 75 億美元收購同類的 OpenRouter；這個領域大約有十來家公司"],
        factsEn: ["Supports OpenAI, Anthropic, Google, Meta and others, routes by cost, speed or benchmark, and shows the reason for each routing decision in the product", "Used internally for three years before launch; Ramp’s own AI spend has grown 21× since summer 2025", "No pricing yet. Veeral Patel, head of applied AI, says the goal so far is just a stable router", "The same month Stripe bought OpenRouter, a similar product, for $7.5 billion; roughly a dozen companies compete in the space"],
        context: "Ramp 賣的是企業卡加報銷軟體，公司用它的卡付錢、用它的軟體管預算。AI 費用對它來說是新的一類支出：按用量計價、每家模型價格不同、而且會被工程師分散在很多帳號裡。Ramp 自己先中招，才做了這個工具。",
        contextEn: "Ramp sells corporate cards plus expense software: companies pay with its card and manage budgets in its app. AI is a new category of spend for it: metered by usage, priced differently by each model, and scattered across engineers’ accounts. Ramp got hit by this itself before building the tool.",
        why: "21 倍不是別人的數字，是 Ramp 自己的。當一家管費用的公司都管不住自家的 AI 帳單，就知道這是一個真的問題。費用管理、企業卡、支付公司都在搶這塊，因為錢從它們的卡上流出去。",
        whyEn: "The 21× figure is not someone else’s; it is Ramp’s own. When an expense-management company cannot control its own AI bill, the problem is real. Expense, corporate-card and payments companies are all after this because the money leaves through their cards.",
        taiwan: "幫公司管 AI 花費，台灣還很少人做。想做專案的話，這個題目的門檻不高，各家模型的用量 API 都是公開的。",
        taiwanEn: "Managing corporate AI spend is still rare in Taiwan. As a project topic the bar is low: every model provider exposes usage APIs.",
        watch: ["Ramp 什麼時候決定 Router 收費方式", "Stripe 買下 OpenRouter 後，會不會把它綁進自己的收款產品"],
        watchEn: ["When Ramp decides how to charge for Router", "Whether Stripe folds OpenRouter into its payments products"],
        quote: "AI 用量是公司成長最快的支出之一，也是最量不出來的一項。",
        quoteEn: "AI use is among companies’ fastest-growing expenses and the one they can least measure.",
        quoteBy: "Rahul Sengottuvelu，Ramp 技術長",
        quoteByEn: "Rahul Sengottuvelu, Ramp CTO",
        term: "代幣費：AI 模型按用量計價的單位，用得多付得多。",
        termEn: "Token fees: the usage-based unit AI models bill in; the more you use, the more you pay.",
        sources: [
  ...
  },
  {
    ...
    lede: "創刊號是三家央行的一週：烏茲別克央行找螞蟻集團談跨境支付，歐洲央行因為 Swift 延期要重排系統時程，澳洲央行則對業界說，帳戶對帳戶支付再不動，我們就自己來。",
    ledeEn: "The first issue is a week of three central banks: Uzbekistan’s sat down with Ant Group on cross-border payments, the ECB had to re-plan a system release after Swift slipped, and Australia’s told the industry that if account-to-account payments don’t move, it will step in.",
    stories: [
      {
        title: "烏茲別克央行與螞蟻集團討論跨境支付合作",
        titleEn: "Uzbekistan’s central bank talks cross-border payments with Ant Group",
        lede: "烏茲別克央行 8 月 27 日在 2026 絲路金融科技論壇上和螞蟻集團的代表會面，談跨境支付和人才培育。只有討論，沒有簽約，也沒有時程。",
        ledeEn: "The Central Bank of Uzbekistan met Ant Group representatives at the Silk Road Finance & Technology Forum 2026 on August 27 to discuss cross-border payments and talent development. Talks only: nothing signed, no timeline.",
        facts: ["談的兩個方向：讓國際旅客在烏國付款更方便，以及讓烏國公民在國外更容易用到支付服務", "也談到讓烏國人參加螞蟻的 10x1000 Tech for Inclusion 培訓計畫", "雙方說會繼續對話，新聞稿沒有任何數字"],
        factsEn: ["Two directions: easier payments for international visitors in Uzbekistan, and better access to payment services for Uzbek citizens abroad", "Also discussed: Uzbek participation in Ant’s 10x1000 Tech for Inclusion training programme", "Both sides said they would keep talking; the release contains no figures"],
        context: "螞蟻集團是支付寶的母公司，這幾年在中亞和東南亞的做法都一樣：先讓當地錢包接進 Alipay+ 的網路，讓中國和其他國家的遊客能掃當地的碼。烏茲別克這幾年在推觀光，央行直接出面談，是想把入口握在自己手上。",
        contextEn: "Ant Group is the parent of Alipay. Its playbook in Central and Southeast Asia has been the same: get local wallets onto the Alipay+ network so visitors from China and elsewhere can scan local QR codes. Uzbekistan has been pushing tourism, and the central bank negotiating directly is a way of keeping the entry point in its own hands.",
        why: "跨境支付的入口正在從銀行移到錢包業者手上。一家央行跳過銀行直接跟支付平台談，說明它也這麼看。",
        whyEn: "The entry point for cross-border payments is moving from banks to wallet operators. A central bank skipping its banks to talk directly to a payment platform suggests it sees it the same way.",
        taiwan: "台灣的電支業者也在做跨境掃碼：街口、LINE Pay 都接了日本或東南亞的通路。差別在台灣是業者自己談，央行沒有出面。烏茲別克這個案例可以拿來對照，看央行介入之後條件會不會不一樣。",
        taiwanEn: "Taiwan’s e-payment operators are doing cross-border QR too: JKOPay and LINE Pay have hooked into Japan and Southeast Asia. The difference is that in Taiwan the operators negotiate themselves; the central bank stays out. Uzbekistan is a case to watch for whether terms differ when a central bank is at the table.",
        watch: ["雙方有沒有簽備忘錄或公布產品", "烏國錢包是否接入 Alipay+"],
        watchEn: ["Whether an MOU or product follows", "Whether Uzbek wallets join Alipay+"],
        quote: "雙方表示，準備繼續對話，推動數位金融與支付技術的實際合作。",
        quoteEn: "The parties expressed their readiness to continue dialogue aimed at further developing practical cooperation in digital finance and payment technologies.",
        quoteBy: "烏茲別克央行新聞稿",
        quoteByEn: "Central Bank of Uzbekistan press release",
      ...
      },
      {
        ...
        lede: "Swift 8 月 27 日宣布延後 2026 年的報文標準更新，隔天歐洲央行說，原訂 11 月的 TARGET Services 更新時程要重新評估。最後決定還沒公布。",
        ledeEn: "Swift said on August 27 it would postpone its 2026 messaging standards release; the next day the ECB said the November TARGET Services release would be reassessed. No decision yet.",
        facts: ["Swift 延的是 ISO 20022 報文格式的移轉，理由是業界要求更多準備時間", "歐元體系正在評估的方案：11 月照原時程更新系統，但延後停用非結構化的郵政地址，並對受影響的參與機構提供緩解措施", "ECB 說會「盡快」公布決定，沒有給日期"],
        factsEn: ["Swift postponed the ISO 20022 message-format migration, citing industry requests for more preparation time", "Option under evaluation: go ahead with the November release but delay retiring unstructured postal addresses, with mitigation for affected participants", "The ECB says it will announce a decision “as soon as possible”, with no date"],
        context: "TARGET Services 是歐洲央行體系的大額支付和證券結算系統，歐元區銀行之間的錢最後都在這裡清算。ISO 20022 是全球銀行報文的新格式，把付款人地址從一行文字改成一格一格的結構化欄位，好處是自動化和反洗錢篩查，代價是每家銀行都要改系統。",
        contextEn: "TARGET Services are the Eurosystem’s large-value payment and securities settlement systems; euro-area interbank money ultimately clears there. ISO 20022 is the new global bank-messaging format, turning a payer’s address from a line of text into structured fields, which helps automation and sanctions screening but means every bank has to change its systems.",
        why: "支付系統的時程是連在一起的。Swift 一延，用 Swift 報文的央行系統就得跟著排。這也說明 ISO 20022 的移轉比大家想的難，連歐洲都要多給時間。",
        whyEn: "Payment-system timelines are chained together. When Swift slips, central-bank systems that use Swift messages have to re-plan. It also shows the ISO 20022 migration is harder than expected, if even Europe needs more time.",
        taiwan: "台灣的銀行同樣要跟上 Swift 的 ISO 20022 時程，跨境匯款的報文都走這套。Swift 延期後，本地銀行的改版排程也會跟著調整。",
        taiwanEn: "Taiwan’s banks follow the same Swift ISO 20022 timeline; cross-border remittance messages run on it. Swift’s delay will shift local upgrade schedules too.",
        watch: ["ECB 對 11 月時程的最終決定", "Swift 的新時程表"],
        watchEn: ["The ECB’s final decision on the November release", "Swift’s new timetable"],
        quote: "Swift 的公告是回應業界要求，希望有更多時間準備這些變更。",
        quoteEn: "Swift’s announcement is a response to the industry’s request for additional time to prepare for the changes planned.",
        quoteBy: "歐洲央行公告",
        quoteByEn: "ECB announcement",
      ...
      },
      {
        ...
        lede: "澳洲央行的支付系統委員會 8 月 27 日開會，歡迎業界圓桌會議發布的帳戶對帳戶支付願景，並說如果業界自己推不動，委員會鼓勵央行出手。",
        ledeEn: "The Reserve Bank of Australia’s Payments System Board met on August 27, welcomed the industry roundtable’s account-to-account payments vision, and said that if industry cannot make coordinated progress it encourages the RBA to step in.",
        facts: ["業界正在寫一份路線圖，列出優先事項、交付項目和時程", "委員會點名要解決的：批次清算系統 BECS 的去向、韌性與備援、批次和拉式付款、帳戶涵蓋率、標準化", "同一天生效的還有 2026 年現金配送框架法"],
        factsEn: ["Industry is drafting a roadmap of priorities, deliverables and timelines", "Issues the Board named: the future of the BECS batch clearing system, resilience and contingency, bulk and pull payments, account reach gaps, standardisation", "The Cash Distribution Framework Act 2026 commenced the same day"],
        context: "澳洲有兩套帳戶對帳戶的系統：老的 BECS 是批次清算，一天跑幾次；新的 NPP 是即時支付。業界討論多年要怎麼把 BECS 退場，但薪資、帳單扣款這些大量批次的東西都還在上面跑，一直沒人敢動。",
        contextEn: "Australia runs two account-to-account systems: the old BECS batch system, which clears a few times a day, and the newer NPP for real-time payments. Industry has debated retiring BECS for years, but payroll and direct debits still run on it in bulk, and nobody has wanted to move first.",
        why: "帳戶對帳戶支付繞過卡組織，商家手續費低很多，所以卡組織和銀行沒有太強的動機推。央行公開說「你們不做我就做」，等於給了期限。",
        whyEn: "Account-to-account payments bypass card networks and cost merchants far less, which is exactly why networks and banks have little incentive to push them. A central bank saying in public that it will act if they don’t is a deadline.",
        taiwan: "台灣的帳戶互轉早就即時，銀行和電支帳戶之間也能轉。澳洲卡在的批次退場和拉式付款（由收款方發起扣款）反而是台灣可以先想的下一步。",
        taiwanEn: "Account transfers in Taiwan are already real-time, including between banks and e-payment accounts. Where Australia is stuck, retiring batch and adding pull payments initiated by the payee, is the next step Taiwan could think about early.",
        watch: ["業界路線圖的發布時間", "BECS 退場有沒有訂日期"],
        watchEn: ["When the industry roadmap is published", "Whether a BECS retirement date is set"],
        quote: "如果業界無法在帳戶對帳戶支付的現代化上協調推進，委員會鼓勵央行考慮進一步行動，以達成符合公共利益的結果。",
        quoteEn: "If the industry is unable to make coordinated progress on modernising A2A payments, the Board encourages the RBA to consider further action to achieve outcomes in the public interest.",
        quoteBy: "澳洲央行支付系統委員會",
        quoteByEn: "RBA Payments System Board",
        term: "帳戶對帳戶（A2A）支付：錢直接從付款人帳戶到收款人帳戶，不經過信用卡組織。",
