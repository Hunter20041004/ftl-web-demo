...
// 短標曾限制 14 字，外商公司名一長就寫不下（例如「JPMorgan 推出代幣化存款」）。放寬到 24 字。
const filled = () => {
...
test("headline accepts up to 24 characters", () => {
  const issue = filled();
  issue.stories[0].headline = "JPMorgan 推出代幣化存款服務"; // 17 字
  assert.ok(issueSchema.safeParse(issue).success);
  issue.stories[0].headline = "一".repeat(24);
  assert.ok(issueSchema.safeParse(issue).success);
});

test("headline rejects 25 characters or empty", () => {
  const issue = filled();
  issue.stories[0].headline = "一".repeat(25);
  assert.ok(!issueSchema.safeParse(issue).success);
